...
def print_map():
	...
	while print_go == True:	
		...
		for y in map_list:
			...
			for x in y:
				if x.type == 0:
					screen.blit(bg_tile, (x_counter*tile_size, counter*tile_size))
				if x.type == 1 and x.water == False:
					screen.blit(floor_tile, (x_counter*tile_size, counter*tile_size))
				if x.type == 2:
					screen.blit(wall_tile, (x_counter*tile_size, counter*tile_size))
				if x.type == 3 and x.water  == False:
					screen.blit(hall_tile, (x_counter*tile_size, counter*tile_size))
				if x.water == True:
					screen.blit(water_tile, (x_counter*tile_size, counter*tile_size))
				x_counter+=1
			counter += 1
	...
	while print_go == True:
		...
		for y in map_list:
			...
			for x in y:
				...
				x_counter+=1
			counter += 1
		screen.blit(destination_tile, (destination[0]*tile_size, destination[1]*tile_size))
		screen.blit(start_tile, (fill_start[0]*tile_size, fill_start[1]*tile_size))
		pygame.display.update()
		events = pygame.event.get()
		for event in events:
			if event.type == pygame.QUIT:
...
def on_move_events(regen_counter, moves_this_turn, enemy_wait, regen_rate, character_health):
	if moves_this_turn:
...
def print_map_game(destination):
	...
	while print_go == True:
		...
		if jumped_this_turn == True:
			...
			enemy_wait = True
			if pygame.time.get_ticks()-jump_time >= 250:
				enemy_wait = False
				print('Enemy moves')
		...
		for y in map_list:
			...
			for x in y:
				...
				if x.type == 0:
					...
					screen.blit(x_tile, (x_counter*tile_size, counter*tile_size))
				if x.type == 1 and x.water == False:
					x_tile = floor_tile
					if x.visible:
						x_tile.set_alpha(x.light)
		...
		if not moves_this_turn:
			if key[pygame.K_s] or key[pygame.K_DOWN]:
				...
				s_count += move_speed
				if s_count >= 10:
					print(pygame.time.get_ticks()-s_time, 'time to move')
					s_time = False
					s_count = 0
					if map_list[character_pos[1]+1][character_pos[0]].type != 2:
						if not [character_pos[0],character_pos[1]+1] in enemy_locations:
		...
		if not moves_this_turn:
			if key[pygame.K_d] or key[pygame.K_RIGHT]:
				...
				d_count = 0
		on_move_events(regen_counter, moves_this_turn, enemy_wait, regen_rate, character_health)
		# if not moves_this_turn:
		# 	if key[pygame.K_w] or key[pygame.K_UP]:
		# 		w_count += move_speed
		# 		if w_count >= 10:
